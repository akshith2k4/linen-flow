...
interface SearchFormWidgetProps {
  data: {
    title: string;
    description?: string;
    search_type: "hotel" | "order";
    api_source: string;
    placeholder?: string;
  };
  threadId?: string; // Optional for future use
}

export function SearchFormWidget({ data }: SearchFormWidgetProps) {
  ...
  const handleSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      const fullUrl = data.api_source.startsWith('/') ? `${backendUrl}${data.api_source}` : data.api_source;
  ...
  const selectResult = (result: any) => {
    ...
    const event = new CustomEvent('sendChatMessage', { 
      detail: { 
        message: `Show me details for ${data.search_type} ID ${result.id}`,
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {data.description && !selectedItem && (
              <p className="text-xs text-text-muted mt-0.5">{data.description}</p>
            )}
          </div>
          {selectedItem && (
          ...
          <>
            <div className="relative">
              <SearchInput
                ...
                placeholder={data.placeholder || `Type ${data.search_type} name to search...`}
                className="text-sm"
              />
              {isSearching && (
              ...
              <div className="space-y-2">
                {results.length > 0 ? (
                  ...
                  <div className="text-center py-6 text-text-muted">
                    ...
                    <p className="text-xs">No {data.search_type}s found matching "{query}"</p>
            ...
            {!hasSearched && (
              <div className="text-center py-5 text-text-muted">
                ...
                <p className="text-xs">Start typing to search for {data.search_type}s</p>
              </div>
            )}
          </>
        )}
